...
export class TeacherProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.API.getTeacherInfo(<any>localStorage.getItem('email')).subscribe(
      data => {
  ...
  editProfile(){
    ...
    if(/[^a-z]/i.test(name1)){
      ...
      let teacher = {correoInstitucional:email,carnet:id,primerNombre:name1,segundoNombre:name2,primerApellido:surname1,segundoApellido:surname2,telefono:phoneNumber};

      this.dialog.open(YesNoPopupComponent,{height:'17vh',width:'20vw',data:{id:17,message:"¿Guardar los cambios?",teacher:teacher}});
    }

  }
 
  deleteProfile(){
    this.dialog.open(YesNoPopupComponent,{height:'17vh',width:'20vw',data:{id:18,message:"¿Eliminar tu perfil?"}});
  }
