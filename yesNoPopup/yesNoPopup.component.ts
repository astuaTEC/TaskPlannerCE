...
export class YesNoPopupComponent implements OnInit {
  ...
  registerStudent(){
    this.API.registerStudent(this.data.student).subscribe(
      data => {},
      error => {
        this.router.navigate(['']);
      }
  ...
  registerTeacher(){
    this.API.registerTeacher(this.data.teacher).subscribe(
      data => {},
      error => {
        this.router.navigate(['']);
      }
  ...
  registerAdmin(){
    this.API.registerAdmin(this.data.admin).subscribe(
      data => {},
      error => {
        window.location.reload();
      }
  ...
  addState(){
    this.API.addType([{nombreTipo:this.data.typeName,nombreEstado:this.data.stateName}]).subscribe(
      data => {},
      error => {
        this.dialog.closeAll();
        this.dialog.open(AdminBoardsPopupComponent,{height:'60vh',width:'60vw',data:{typeName:this.data.typeName}});
      }
    )
  }

  deleteStateFromType(){
    this.API.deleteStateFromType(this.data.typeName,this.data.stateName).subscribe(
      data =>{},
      error => {
        this.dialog.closeAll();
        this.dialog.open(AdminBoardsPopupComponent,{height:'60vh',width:'60vw',data:{typeName:this.data.typeName}});
  ...
  deleteAdminType(){
    ...
    this.API.deleteAdminType(this.data.typeName).subscribe(
      data =>{},
      error => {
        window.location.reload();
      }
